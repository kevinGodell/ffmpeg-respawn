  ...
  constructor(options) {
    ...
    if (FfmpegRespawn._checkLoglevel(options.logLevel)) {
      if (typeof options.stderrLogs === 'function' && options.stderrLogs.length > 0) {
        ...
        this._stdioPipes.push({
          stdioIndex: 2,
          destination: FfmpegRespawn._createWritable((data) => console.log(`stderr:${data}`))
        });
      }
  ...
  _onExit(code, signal) {
    ...
    if (this._running === true) {
      if (this._reSpawnCounter >= this._reSpawnLimit) {
        ...
        this.emit(
          'fail',
          `Ffmpeg unable to get progress from input source after ${this._reSpawnCounter} attempts at respawning.`
        );
      } else {
