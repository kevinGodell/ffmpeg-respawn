    ...
    constructor(options) {
        ...
        for (let i = 0; i < this._params.length; i++) {
            if (this._params[i] === '-i' && (this._params[i + 1] === 'pipe:0' || this._params[i + 1] === '-' || this._params[i + 1] === 'pipe:')) {
                ...
                this._stdio[2] = 'inherit';//will have to make this pipe if piping to writestream
                //todo create writepipe to get and send out logs
        ...
        this._logCallback('hello');

        const logCallback = this._logCallback.bind(this);
        const logPipe = new Writable({
            write(chunk, encoding, callback) {
        ...
        this._pipes.push({stdioIndex: 2, destination: logPipe});*/
