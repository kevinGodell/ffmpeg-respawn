...
assert(new FfmpegRespawn({ params: ['-i', 'in', 'out'] }).params === '-loglevel quiet -progress pipe:1 -i in out');
...
}, /Params error: stdout\/stdio\[1]\/pipe:1 is reserved for internal progress monitoring/);
...
assert.throws(() => {
  new FfmpegRespawn({ params: ['-i', 'in', 'pipe:3'], pipes: [{ stdioIndex: 4, destination: writable }] });
}, /Pipes error: pipes array did not have a matching pipe or callback for pipe:3./);

assert.throws(() => {
  new FfmpegRespawn({ params: ['-i', 'in', 'pipe:3'], pipes: [{ stdioIndex: 3, destination: null }] });
